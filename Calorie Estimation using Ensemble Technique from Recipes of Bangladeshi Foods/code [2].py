...
train_texts, temp_texts, train_labels, temp_labels = train_test_split(
    df['Recipe'].tolist(),
    df['Approximate Calorie per Serving'].values,
    test_size=0.3,
    random_state=42
)
...
class TransformerRegressor(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, labels=None):
        ...
        cls_output = outputs.pooler_output if hasattr(outputs, 'pooler_output') else outputs.last_hidden_state[:, 0]
...
# ---------------------------------------------------
# Training Loop for Each Transformer
# ---------------------------------------------------
