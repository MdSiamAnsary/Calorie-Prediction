# -*- coding: utf-8 -*-
"""
Ablation A1: CLS Token Pooling (no attention pooling)
Encoder is fine-tuned end-to-end.
Replace this result in Table IV row: "w/o Attention Pooling (CLS)"
"""
...
class CLSPoolingRegressor(nn.Module):
...
for fold, (train_idx, test_idx) in enumerate(kf.split(texts), 1):
    ...
    model = CLSPoolingRegressor(MODEL_NAME).to(device)

    training_args = TrainingArguments(
        output_dir=f"./ablation_cls_fold_{fold}",
        num_train_epochs=10,
